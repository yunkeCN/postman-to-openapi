...
const OUTPUT_PATH = "./openAPIRes.json"
...
describe("Cli specs", function () {
  ...
  it("should print an error when result file cannot be created", async function () {
    await rejects(execa("node", [cliPath, COLLECTION_BASIC, "-f", "./no_exist/result.json"]), {
      name: "Error",
      stderr: "Error: ENOENT: no such file or directory, open './no_exist/result.json'",
      exitCode: 1,
    })
  })
